...
static irqreturn_t ts_work_func(int irq, void *dev_id)
{
    ...
    else if(pressed_num == 1)
    {
        ...
        FID_in1stbuf = buf[6 + ADDR_OFFSEET] >>4;
        SID_in1stbuf = FID_in1stbuf-1;
    ...
    else
    {
        ...
        FID_in2ndbuf = buf[6 + ADDR_OFFSEET] & 0xf;
        SID_in2ndbuf = FID_in2ndbuf-1;
    }

    fingerInfo[SID_in1stbuf].x = (buf[1 + ADDR_OFFSEET] << 8) |buf[2 + ADDR_OFFSEET];
	fingerInfo[SID_in1stbuf].y = (buf[3 + ADDR_OFFSEET] << 8) |buf[4 + ADDR_OFFSEET];
	fingerInfo[SID_in1stbuf].z = buf[5 + ADDR_OFFSEET];
	fingerInfo[SID_in1stbuf].id = FID_in1stbuf;
	fingerInfo[SID_in2ndbuf].x = (buf[7 + ADDR_OFFSEET] << 8) |buf[8 + ADDR_OFFSEET];
	fingerInfo[SID_in2ndbuf].y = (buf[9 + ADDR_OFFSEET] << 8) |buf[10 + ADDR_OFFSEET];
	fingerInfo[SID_in2ndbuf].z = buf[11 + ADDR_OFFSEET];
